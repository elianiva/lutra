...
- **Open via drop.** Run `node scripts/verify.mjs --scenario open`. The harness builds a real `File` from `fixtures/sample.png`, adds it to a `DataTransfer`, and dispatches `dragenter`/`dragover`/`drop` on `[data-gallery-drop-zone]`.
- **Result — editor reached.** The `open-photo` step asserts `location.href` matches `/edit/:id`; the `editor-canvas` step asserts a `<canvas>` with the image's dimensions (`1200×800` for the fixture) exists.
- **Proof.** `smoke-gallery.png` (loaded gallery) and `smoke-editor.png` (editor with the tool rail and `LAYERS` panel), plus `open-result.json` recording the reached URL and canvas size.
...
- The main canvas is black under software WebGPU — that is expected here and is not evidence of a failed open. Prove the open by the URL change and the canvas element, and processing by applying a LUT (see apply-lut).
