...
function findIndex(chain: readonly Layer[], id: LayerId): number {
...
export function removeLayer(chain: readonly Layer[], id: LayerId): Layer[] {
...
export function reorderLayer(chain: readonly Layer[], id: LayerId, newIndex: number): Layer[] {
  const idx = findIndex(chain, id)
  if (idx === -1) {
    return [...chain]
  }
...
export function updateLayerParam(chain: readonly Layer[], patch: LayerPatch): Layer[] {
...
export function toggleLayerVisibility(chain: readonly Layer[], id: LayerId): Layer[] {
  const idx = findIndex(chain, id)
  if (idx === -1) {
    return [...chain]
  }
...
export function replaceChain(_chain: readonly Layer[], layers: Layer[]): Layer[] {
  return [...layers]
}
