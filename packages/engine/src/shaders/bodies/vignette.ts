...
// negative brightens them. uv maps [0,1] → [-1,1]; aspect ratio is
// preserved so the vignette stays circular.
export const renderVignette: BodyRenderer = (i) => `
// vignette
{
  let uv = vec2<f32>(f32(coord.x), f32(coord.y)) / u_resolution * 2.0 - 1.0;
  let dist = length(uv);
