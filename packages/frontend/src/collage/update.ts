...
const delegate = (
  ...
  const [dialogModel, commands] = ExportDialog.update(
    model.exportDialog,
    message,
  );
  return [
  ...
  M.value(message).pipe(
    ...
    M.tags({
      ...
      ThumbsMeasured: ({ sizes }): UpdateReturn => [
        {
          ...
          sizes: [
            ...model.sizes.filter(
              (s) => !sizes.some((n) => n.editId === s.editId),
            ),
            ...sizes,
          ],
        },
      ...
      ChangedColumns: ({ columns }) =>
        mutate(model, (c) => ({
          ...
          layout: {
            ...
            columns: clamp(
              Math.round(columns),
              LAYOUT_BOUNDS.minColumns,
              LAYOUT_BOUNDS.maxColumns,
            ),
          },
        })),
      ChangedRows: ({ rows }) =>
        mutate(model, (c) => ({
          ...
          layout: {
            ...
            rows: clamp(
              Math.round(rows),
              LAYOUT_BOUNDS.minRows,
              LAYOUT_BOUNDS.maxRows,
            ),
          },
        })),
      ChangedGutter: ({ gutter }) =>
        mutate(model, (c) => ({
          ...
          layout: {
            ...
            gutter: clamp(
              Math.round(gutter),
              LAYOUT_BOUNDS.minGutter,
              LAYOUT_BOUNDS.maxGutter,
            ),
          },
        })),
      ChangedFrameRatio: ({ frameRatio }) =>
        mutate(model, (c) => ({
          ...
          layout: {
            ...
            frameRatio: clamp(
              frameRatio,
              LAYOUT_BOUNDS.minFrameRatio,
              LAYOUT_BOUNDS.maxFrameRatio,
            ),
          },
        })),
      ToggledBackground: () =>
      ...
      RemovedTile: ({ index }) => {
        ...
        const [nextModel, commands] = mutateWithUndo(
          model,
          "Removed photo",
          (c) => ({
            ...c,
      ...
      GotDragMessage: ({ message }) => {
        const [drag, dragCommands, out] = DragAndDrop.update(
          model.drag,
          message,
        );
        ...
        return Option.isSome(out)
          ? M.value(out.value).pipe(
              ...
              M.tag(
                "Reordered",
                ({ itemId, fromIndex, toContainerId, toIndex }) => {
                  const collage = collageOf(base[0]);
                  const sourceIndex =
                    collage?.tiles.findIndex((t) => t.editId === itemId) ??
                    fromIndex;
      ...
      PanMoved: ({ screenX, screenY }) => {
        ...
        const cellAspect =
          model.cellPx.width / Math.max(1, model.cellPx.height);
        ...
        const framing = panned(
          model.framingDraft.framing,
          imageAspect,
          cellAspect,
          dx,
          dy,
      ...
      ResetFraming: ({ index }) => {
        ...
        return mutateWithUndo(model, "Framing reset", (c) => ({
          ...
          tiles: c.tiles.map((t, i) =>
            i === index ? { ...t, framing: defaultTileFraming() } : t,
          ),
        }));
      },
      CellMeasured: ({ width, height }) => {
        ...
        return [
          same ? model : { ...model, cellPx: { width, height } },
          [],
          Option.none(),
        ];
      },
      ...
      ExportRequested: () => {
        ...
        const [dialogModel, dialogCommands] = ExportDialog.open(
          model.exportDialog,
        );
        return [
