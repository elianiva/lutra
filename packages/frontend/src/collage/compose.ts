...
export const effectiveRowCount = (
  ...
  const requested = Number.isFinite(layout.rows)
    ? Math.max(1, Math.round(layout.rows))
    : 1;
  return Math.max(
    requested,
    rowCount(count, Math.max(1, Math.round(layout.columns))),
  );
};
...
export const frameSize = (
  ...
  const ratio =
    Number.isFinite(layout.frameRatio) && layout.frameRatio > 0
      ? layout.frameRatio
      : 1;
  return ratio >= 1
