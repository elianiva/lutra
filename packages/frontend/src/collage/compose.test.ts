...
const layoutWith = (
  overrides?: Partial<Pick<CollageLayout, 'columns' | 'rows' | 'gutter' | 'frameRatio'>>,
) => ({
...
describe('effectiveRowCount (docs/adr/0009-collage)', () => {
  ...
  it('a nonsensical stored row count falls back to the derived grid', () => {
    expect(effectiveRowCount(layoutWith({ columns: 2, rows: Number.NaN }), 5)).toBe(3)
  ...
  it('an explicit M×N shapes cellSize and the composed height', () => {
    ...
    const singleRow = gridRects(layoutWith({ columns: 2, rows: 1, gutter: 0 }), 2)
