...
const layoutWith = (
  overrides?: Partial<
    Pick<CollageLayout, "columns" | "rows" | "gutter" | "frameRatio">
  >,
) => ({
...
describe("frameSize", () => {
  ...
  it("a portrait frame narrows the width; a landscape frame widens it", () => {
    ...
    expect(frameSize({ frameRatio: 16 / 9 }).width).toBe(
      Math.round((FRAME_SHORT_EDGE * 16) / 9),
    );
  });

  it("a nonsensical ratio falls back to square", () => {
    expect(frameSize({ frameRatio: 0 })).toEqual(square(FRAME_SHORT_EDGE));
    expect(frameSize({ frameRatio: Number.NaN })).toEqual(
      square(FRAME_SHORT_EDGE),
    );
  });
});
...
describe("cellSize / gridRects", () => {
  ...
  it("a 4:5 frame with zero gutter yields 4:5 output and cells (within a pixel)", () => {
    ...
    const cell = cells[0]!;
    expect(Math.abs(cell.width / cell.height - 0.8)).toBeLessThan(
      1 / FRAME_SHORT_EDGE,
    );
  });
...
describe("effectiveRowCount (docs/adr/0035)", () => {
  it("legacy rows (defaulting to 1) derive from the tile count as before", () => {
    ...
    expect(effectiveRowCount(layoutWith({ columns: 3 }), 0)).toBe(1);
  });

  it("an explicit row count is honored when capacity suffices", () => {
    // 2×2 with 3 photos: one spare cell.
    expect(effectiveRowCount(layoutWith({ columns: 2, rows: 2 }), 3)).toBe(2);
    // A tall single column.
    expect(effectiveRowCount(layoutWith({ columns: 1, rows: 4 }), 2)).toBe(4);
    // One wide row.
    expect(effectiveRowCount(layoutWith({ columns: 3, rows: 1 }), 3)).toBe(1);
  });

  it("surplus photos grow the grid instead of vanishing", () => {
    expect(effectiveRowCount(layoutWith({ columns: 2, rows: 2 }), 7)).toBe(4);
  });

  it("a nonsensical stored row count falls back to the derived grid", () => {
    expect(
      effectiveRowCount(layoutWith({ columns: 2, rows: Number.NaN }), 5),
    ).toBe(3);
    expect(effectiveRowCount(layoutWith({ columns: 2, rows: 0 }), 5)).toBe(3);
  });

  it("an explicit M×N shapes cellSize and the composed height", () => {
    // 2×2 with 3 photos: cells divide a two-row frame; the last cell stays background.
    const layout = layoutWith({ columns: 2, rows: 2, gutter: 0 });
    const { width, height, cells } = gridRects(layout, 3);
    expect(cells).toHaveLength(3);
    expect(height).toBe(2 * cells[0]!.height);
    expect(width).toBe(2 * cells[0]!.width);
    // Same cell size either way — cellSize and gridRects agree.
    expect(cells[0]).toMatchObject(cellSize(layout, 3));
    // The frame itself stays pinned to the ratio; doubling the rows halves
    // each cell's height instead of stretching the output.
    const singleRow = gridRects(
      layoutWith({ columns: 2, rows: 1, gutter: 0 }),
      2,
    );
    expect(height).toBeCloseTo(singleRow.height, -1);
    expect(cells[0]!.height).toBeCloseTo(singleRow.cells[0]!.height / 2, -1);
  });
});
