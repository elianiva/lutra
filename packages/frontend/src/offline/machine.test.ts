...
describe('offline machine', () => {
  ...
  it('a run beginning leaves Idle for Filling', () => {
    const result = step(Idle(), OfflineMessage.OfflineFillStarted({ done: 0, total: 593 }))
    expect(result._tag).toBe('Transitioned')
    expect(result.state).toEqual(Filling())
  })

  it('completing the run reaches Ready', () => {
    const result = step(Filling(), OfflineMessage.OfflineFillComplete())
    expect(result._tag).toBe('Transitioned')
    expect(result.state).toEqual(Ready())
  })

  it('the network dropping pauses a Filling run', () => {
    const result = step(Filling(), OfflineMessage.ConnectivityChanged({ online: false }))
    expect(result._tag).toBe('Transitioned')
    expect(result.state).toEqual(Paused())
  })

  it('the loop noticing the drop itself pauses too', () => {
    const result = step(Filling(), OfflineMessage.OfflineFillPaused())
    expect(result.state).toEqual(Paused())
  })
  ...
  it('an online event does NOT unpause — only the loop resumes itself', () => {
    ...
    const result = step(Paused(), OfflineMessage.ConnectivityChanged({ online: true }))
    expect(result._tag).toBe('Ignored')
    expect(result.state).toEqual(Paused())
  })

  it('the loop resuming returns to Filling', () => {
    const result = step(Paused(), OfflineMessage.OfflineFillResumed())
    expect(result.state).toEqual(Filling())
  })

  it('quota stops the run into QuotaError from Filling or Paused', () => {
    expect(step(Filling(), OfflineMessage.OfflineQuotaError({ message: 'full' })).state).toEqual(
      QuotaError(),
    )
    expect(step(Paused(), OfflineMessage.OfflineQuotaError({ message: 'full' })).state).toEqual(
      QuotaError(),
    )
  })

  it('a persist-retry run leaves QuotaError via Started', () => {
    const result = step(QuotaError(), OfflineMessage.OfflineFillStarted({ done: 0, total: 593 }))
    expect(result.state).toEqual(Filling())
  })

  it('Ready ignores everything, including a second completion', () => {
    const result = step(Ready(), OfflineMessage.OfflineFillComplete())
    expect(result._tag).toBe('Ignored')
    expect(result.state).toEqual(Ready())
  })

  it('Idle ignores file events and offline flips', () => {
    expect(step(Idle(), OfflineMessage.ConnectivityChanged({ online: false })).state).toEqual(
      Idle(),
    )
    expect(step(Idle(), OfflineMessage.OfflineFillPaused()).state).toEqual(Idle())
  })
