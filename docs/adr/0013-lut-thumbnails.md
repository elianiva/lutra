...
### D3 — The thumb worker is the encode-worker pattern, one request per LUT

`thumbs/worker.ts` + `worker-layer.ts` mirror `encode/` exactly: a stateless module worker, per-request `{ id, image, cube }` posts with transferred replies, a Deferred pending map, `Effect.runFork` event routing, and a terminate-on-teardown finalizer. The main thread downscales the photo once per group visit (a 200×200 center cover-crop, matching the bar's square `object-cover` presentation — 200px because the thumbs render at 96px CSS and 100px would be soft on 2× displays) and transfers the 160KB `ImageData` with each request. The worker applies the LUT at full strength and encodes JPEG q85.
...
- Group generation costs ~100–300ms of worker time and 4–8MB of fetches on first visit; the worker never touches the GPU queue, so slider-drag renders are unaffected.
