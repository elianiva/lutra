...
- **P6 export/save mop-up** — `encode/worker-layer.ts` transfers the `ImageData.data.buffer` (`worker.postMessage(req, [buffer])`) so the 6k/12Mpx export snapshot (~24–92 MiB) is not structured-cloned; guard falls back on non-transferable/SharedArrayBuffer. `SaveEdit`'s thumbnail derives from the same preview-sized snapshot (FHD-class, ~11 MiB), so no 91 MiB double allocation; the full-res native path is export-only on a detached canvas (`SnapshotForExport`). `canvas-stage.ts:CompareDivider` coalesces `pointermove` via rAF (one `ChangedSplitPosition` per frame), and `editor/update.ts` coalesces `PresentFrame` (one blit in flight, `presentPending`/`pendingPresent` flush on `FramePresented`) — divider drag at 240 Hz emits ≤60 blits, mirrors `renderPending`/`revision` coalescing. `ChangedCompareMode` bypasses coalescing (discrete clicks).
