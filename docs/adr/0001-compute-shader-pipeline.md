...
**Multi-pass evolution (added 2026-07):** The original assembler merged every layer into a single compute pass per pixel. That made texture-sampling layers (chromatic aberration) sample the *source* image instead of the accumulated result of earlier layers, and it forced sRGB-encoded texels into the linear pipeline. The assembler now emits one compute pass per layer; passes ping-pong through linear-light rgba16float intermediates and only the last pass encodes back to sRGB. A dedicated linearize pass is inserted ahead of the first sampling layer. Cost: N dispatches per frame instead of 1; benefit: correct chain semantics for any future neighbor-sampling body (real clarity, FBM grain).
